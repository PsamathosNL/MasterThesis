...
After a pulse time corresponding to one electron beam pulse \red{I don't know how to say this nicely) the voltage and current were dropped to zero. This was achieved using the setup seen in figure \ref{fig:circuitsetup}.
...
In the setup, a capacitor $C$ (\SI{1000}{\micro\farad}) was charged to the beam voltage $U_b$ using a high-voltage DC power supply via a resistor $R_\text{charge}$ (14\,\k$\Omega$) to limit the current drawn from the power supply.
A high-voltage switch (Behlke model \red{xxx}) was used to to discharge the capacitor via the rod and a resistor $R$. The resistor $R$ (between 300 and 1200\,$\Omega$ determines the current for a set beam voltage.
%---------- work in progress -----------
