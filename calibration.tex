...
The beam potential $U_b$ was also varied to determine any dependencies of the position monitor's behaviour on the charge density inside. Then, the response of the four electrodes was measured for a range of positions.

\subsection{Translation}
\label{sec:calibration/translation}
The beam position monitor must be accurately moved with respect to the rod and the rotation of the BPM with respect to the rod axis must be controlled in order for the rod and BPM axes to be aligned.
In the setup shown in figure \ref{fig:schem_translation}, the BPM was held by a metal holder that was firmly placed on a 2-axis translation stage. This allowed the monitor to be moved with up to \SI{10}{\micro\meter} accuracy with respect to the rod. The rod was kept in a fixed position.
